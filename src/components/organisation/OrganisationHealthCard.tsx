  ...
  return (
    <section className="rounded-3xl border border-[var(--color-outline-variant)] bg-[var(--color-surface-container-lowest)] p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--color-info)]">Organisation Health</p>
          <h2 className="mt-2 text-2xl font-black tracking-tight text-[var(--color-on-surface)]">{healthScore}%</h2>
          <p className="mt-1 text-sm text-[var(--color-on-surface-variant)]">Weighted from engagement, readiness, progress, evidence, and opportunities.</p>
        </div>
      ...
      <div className="mt-6 space-y-4">
        {metrics.map((metric) => (
          <div key={metric.label}>
            <div className="mb-2 flex justify-between text-sm font-bold text-[var(--color-on-surface)]">
