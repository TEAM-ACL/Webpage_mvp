...
export default function Admin() {
  return (
    <DashboardShell>
      <PageHeader
        title="Monitor platform health and guide VisionTech operations"
        description="Review user activity, onboarding performance, matching quality, workspace signals, and items needing admin attention."
        badge={
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${activityBadge(item.type)}`}>{item.type}</span>
                      </div>
              ...
              {flaggedItems.map((item) => (
                <div key={item.id} className="rounded-2xl bg-[var(--color-surface-container-low)] p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-semibold text-[var(--color-on-surface)]">{item.title}</h3>
                    <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${flagBadge(item.status)}`}>{item.status}</span>
                  </div>
