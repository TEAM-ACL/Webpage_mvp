...
export default function ResetPassword(): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (callbackType === "recovery" && (callbackError || callbackErrorCode)) {
      const expired = callbackErrorCode === "otp_expired";
      const message = expired
        ? "This password reset link has expired. Please request a new password reset email."
        : callbackErrorDescription || "This password reset link is invalid. Please request a new password reset email.";
      setTokenMissingMessage(message);
      return;
    }
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {tokenMissingMessage && (
              <div className="space-y-3">
                ...
                <Link
                  to="/forgot-password"
                  className="inline-flex items-center rounded-lg border border-amber-300 bg-white px-3 py-2 text-sm font-semibold text-amber-800 hover:bg-amber-50"
                >
